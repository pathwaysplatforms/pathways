-- Development seed data: countries, categories, sample UK pathways, document requirements.
-- Uses ON CONFLICT DO NOTHING so this file is safe to apply more than once.
...
-- Document requirements: UK Skilled Worker Visa
...
values
  (
    ...
    'Your current, valid passport or travel document showing your identity and nationality',
    'passport', true, '6 months',
    '{"min_validity_months": 6}', 1
  ),
  (
    (select id from pathway),
    'Certificate of Sponsorship',
    'A valid Certificate of Sponsorship reference number from your UK employer, confirming the job role and salary',
    'certificate_of_sponsorship', true, null,
    '{"must_not_be_expired": true}', 2
  ),
  (
    (select id from pathway),
    'English language evidence',
    'Proof of English ability at B1 level or above — approved test result, UK degree, or exemption',
    'english_language_test', true, '2 years',
    '{"approved_tests": ["IELTS", "TOEFL", "PTE"]}', 3
  ),
  (
    (select id from pathway),
    'Financial evidence',
    'Bank statements showing at least £1,270 held for 28 consecutive days',
    'bank_statement', true, '1 month',
    '{"min_balance_gbp": 127000, "consecutive_days": 28}', 4
  ),
  (
    (select id from pathway),
    'Tuberculosis test results',
    'Required if you are from a country where TB testing is mandatory for UK visa applications',
    'tuberculosis_test', false, '6 months',
    null, 5
  )
on conflict do nothing;

-- Document requirements: UK Student Visa
...
values
  (
    (select id from pathway),
    'Valid passport',
    'Your current, valid passport showing your identity, nationality, and photograph',
    'passport', true, '6 months',
    '{"min_validity_months": 6}', 1
  ),
  (
    ...
    'A CAS reference number from your licensed student sponsor confirming your place on an approved course',
    'confirmation_of_acceptance', true, null,
    '{"must_not_be_expired": true}', 2
  ),
  (
    (select id from pathway),
    'English language test results',
    'Proof of English ability at the level required by your course — IELTS UKVI or equivalent',
    'english_language_test', true, '2 years',
    '{"approved_tests": ["IELTS UKVI", "TOEFL iBT", "PTE Academic"]}', 3
  ),
  (
    (select id from pathway),
    'Financial evidence',
    'Proof of funds to cover tuition fees and living costs for at least one academic year',
    'bank_statement', true, '1 month',
    '{"consecutive_days": 28}', 4
  ),
  (
    (select id from pathway),
    'Academic transcripts and qualifications',
    'Certified copies of academic certificates and transcripts supporting your course application',
    'academic_transcript', true, null,
    null, 5
  ),
  (
    (select id from pathway),
    'ATAS clearance certificate',
    'Academic Technology Approval Scheme certificate — required for certain science, engineering, and technology subjects',
    'atas_certificate', false, null,
    null, 6
  )
on conflict do nothing;

-- Document requirements: UK Global Talent Visa
...
values
  ...
  (
    (select id from pathway),
    'Endorsement letter',
    'An endorsement letter from an approved endorsing body confirming you meet the criteria for exceptional talent or promise',
    'endorsement_letter', true, '3 months',
    '{"approved_bodies": ["Tech Nation", "British Academy", "Arts Council England", "Royal Society"]}', 2
  ),
  (
    (select id from pathway),
    'Evidence of exceptional talent or promise',
    'Portfolio of work, publications, awards, citations, or other evidence demonstrating field-leading contributions',
    'evidence_portfolio', true, null,
    null, 3
  ),
  (
    (select id from pathway),
    'CV and professional profile',
    'Detailed CV covering education, employment history, and achievements relevant to your field',
    'cv', true, null,
    null, 4
  )
on conflict do nothing;
